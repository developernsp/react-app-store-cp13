...
class AppStore extends Component {
  state = {category: 'SOCIAL', searchInput: ''}
  ...
  render() {
    ...
    const appCategoryList = appsList.filter(app => app.category === category)

    const filteredAppsList = appCategoryList.filter(app =>
          ...
          <ul className="tab-container">
            {tabsList.map(item => (
              <TabItem
                tabList={item}
                key={item.tabId}
                isActive={category === item.tabId}
                onBtn={this.onCategory}
              />
            ))}
          </ul>

          <ul className="app-items-container">
            {filteredAppsList.map(appItem => (
              <AppItem appItem={appItem} key={appItem.appId} />
            ))}
          </ul>
        </div>
      </div>
